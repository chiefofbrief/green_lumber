...
Persistent context also requires persistent memory, the capability for an AI system to store information across sessions. This is a non-trivial technical problem: retaining more context adds real infrastructure cost, and it's compounded by the fact that effective memory isn't just about storage, but retrieval (surfacing the right prior context at the right moment). Persistent context may require adjustments to cloud infrastructure, which was built for stateless execution. Most software is stateless; each request is handled independently, with no memory of prior requests, which makes it easy to scale up or down on demand ('elastic'). Stateful systems retain information between requests and need to route users back to infrastructure that holds their state. Cloud infrastructure may also need to adapt from deterministic to probabilistic execution (varying outcomes), although initial AI use cases are mainly focused on verifiable tasks which are closer to deterministic systems. Observability (monitoring what is being done) and governance (controlling what can be done) will continue to be critical.

This runtime complexity compounds with routing decisions, including model routing and compute routing. As the number of models increases, requests will increasingly be directed to particular models based on factors like cost, complexity, domain, etc (assuming users do not interact with vertical models directly). Tasks will also increasingly be routed to accelerators based on availability, cost, and latency requirements as more compute capacity is added. If there is an oversupply of compute, which as already stated is highly likely at some point, then 'winning' during routing will become increasingly important for compute providers. 
